...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_NAUTOBOT_CONNECTIVITY_MATRIX_xxx
namespace = Collection("nautobot_connectivity_matrix")
namespace.configure(
    {
        "nautobot_connectivity_matrix": {
            "nautobot_ver": "3.0.0",
            "project_name": "nautobot-connectivity-matrix",
            "python_ver": "3.12",
            "local": False,
...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.nautobot_connectivity_matrix.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.nautobot_connectivity_matrix.compose_http_timeout,
        "NAUTOBOT_VER": context.nautobot_connectivity_matrix.nautobot_ver,
        "PYTHON_VER": context.nautobot_connectivity_matrix.python_ver,
        **kwargs.pop("env", {}),
    }
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.nautobot_connectivity_matrix.project_name}",
        f'--project-directory "{context.nautobot_connectivity_matrix.compose_dir}"',
    ]

    for compose_file in context.nautobot_connectivity_matrix.compose_files:
        compose_file_path = os.path.join(context.nautobot_connectivity_matrix.compose_dir, compose_file)
...
def run_command(context, command, service="nautobot", **kwargs):
    ...
    if is_truthy(context.nautobot_connectivity_matrix.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.nautobot_connectivity_matrix.python_ver}...")
...
def _ensure_creds_env_file(context):
    ...
    if not os.path.exists(
        os.path.join(context.nautobot_connectivity_matrix.compose_dir, "creds.env")
    ):
        ...
        shutil.copy(
            os.path.join(
                context.nautobot_connectivity_matrix.compose_dir, "creds.example.env"
            ),
            os.path.join(context.nautobot_connectivity_matrix.compose_dir, "creds.env"),
        )
...
def _get_docker_nautobot_version(context, nautobot_ver=None, python_ver=None):
    ...
    if python_ver is None:
        python_ver = context.nautobot_connectivity_matrix.python_ver
    dockerfile_path = os.path.join(context.nautobot_connectivity_matrix.compose_dir, "Dockerfile")
...
def lock(context, check=False, constrain_nautobot_ver=False, constrain_python_ver=""):
    ...
    if constrain_nautobot_ver:
        ...
        try:
            ...
            command = f"poetry add --lock git+https://github.com/nautobot/nautobot.git#{context.nautobot_connectivity_matrix.nautobot_ver}"
            if constrain_python_ver:
                command += f" --python {constrain_python_ver}"
            run_command(context, command)
    else:
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations nautobot_connectivity_matrix"
...
def pylint(context):
    ...
    command = f"{base_pylint_command} nautobot_connectivity_matrix"
    if not run_command(context, command, warn=True):
        exit_code = 1

    # run the pylint_django migrations checkers on the migrations directory, if one exists
    migrations_dir = Path(__file__).absolute().parent / Path("nautobot_connectivity_matrix") / Path("migrations")
    if migrations_dir.is_dir():
        migrations_pylint_command = (
            ...
            " nautobot_connectivity_matrix.migrations"
        )
...
def djhtml(context, check=False):
    ...
    command = "djhtml -t 4 nautobot_connectivity_matrix/templates/"
...
def unittest(  # noqa: PLR0913
    context,
    keepdb=False,
    label="nautobot_connectivity_matrix",
    failfast=False,
    buffer=True,
    pattern="",
    verbose=False,
...
def tests(context, failfast=False, keepdb=False, lint_only=False):
    ...
    if not is_truthy(context.nautobot_connectivity_matrix.local):
