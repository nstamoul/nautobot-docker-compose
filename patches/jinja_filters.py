...
def get_vlan_data(item, vlan_mapping, tag_type):  # pylint: disable=too-many-return-statements
    ...
    if current_item and int_mode:
        ...
        if tag_type == "tagged":
            ...
            expanded_vids = list(chain.from_iterable([vlanconfig_to_list(vlan_stanza) for vlan_stanza in trunk_vlans]))
            filtered_vlans = [
                {"id": str(vid), "name": vlan_mapping.get(str(vid), f"VLAN{str(vid).zfill(4)}")}
                for vid in expanded_vids
                if str(vid) in vlan_mapping  # Only include VLANs that exist in VLAN database
            ]

            # Log filtering results for debugging
            if len(filtered_vlans) < len(expanded_vids):
                logger.info(f"🔧 VLAN FILTERING: {len(filtered_vlans)} VLANs exist in VLAN database out of {len(expanded_vids)} allowed on trunk")
