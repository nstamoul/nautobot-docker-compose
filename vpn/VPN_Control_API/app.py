...
def get_status_for_context(context: SlotContext) -> SlotStatus:
    ...
    if vpn_id:
        ...
        vpn_type = get_env_var(vpn_id, "VPN_TYPE")
        vpn_client = get_env_var(vpn_id, "VPN_CLIENT")
    else:
        vpn_type = None
        vpn_client = None
    ...
    vpn_running = vpn_id is not None
    ...
    return SlotStatus(
        ...
        status=slot_state(
            appliance_exists=bool(appliance_containers),
...
def _discover_running_contexts() -> list[SlotContext]:
    ...
    contexts_by_slot: dict[str, SlotContext] = {}
    try:
        result = run_command(
            [
                "docker",
    ...
    *,
    ...
    try:
        run_command(
            compose_command(
                context,
                [VPN_SERVICE_COMPOSE_FILE, VPN_QUEUE_COMPOSE_FILE],
                [
                    "up",
                    "-d",
                    "--no-deps",
...
def start_endpoint(payload: StartRequest) -> StatusResponse:
    ...
    if profile.vpn_client:
        env["VPN_CLIENT"] = profile.vpn_client
    if profile.profile_name:
        env["CISCO_PROFILE_NAME"] = profile.profile_name
    if profile.usergroup:
        env["CISCO_USERGROUP"] = profile.usergroup
    ...
    if profile.vpn_client:
        cmd.extend(["--vpn-client", profile.vpn_client])
    if profile.profile_name:
