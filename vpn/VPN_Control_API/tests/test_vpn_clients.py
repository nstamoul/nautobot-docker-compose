...
def app_module(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("VPN_HOST_SLOT_STATE_FILE", str(tmp_path / "host-slots.json"))
...
def test_cisco_anyconnect_defaults_to_openconnect_when_vpn_client_metadata_is_missing(app_module, monkeypatch):
    ...
    assert profile.client_runtime == "container"
    assert profile.worker_network_mode == "service-vpn"


def test_start_endpoint_uses_official_client_host_backend_and_host_worker_compose(app_module, monkeypatch):
    context = app_module.slot_context("e-Trikala")
    calls = {"host_start": [], "worker_scale": []}
    ...
    up_response = app_module.StatusResponse(
        vpn_running=True,
        customer=context.customer,
        tunnel_interface="cisco-secure-client",
        worker_running=True,
        slots=[
            app_module.SlotStatus(
                customer=context.customer,
                slot=context.slug,
                queue_name=context.queue_name,
                project_name=context.project_name,
    ...
    def fake_start_host_secure_client(context, profile, *, banner_response=None, env=None):
        calls["host_start"].append((context, profile, banner_response, env))

    def fake_ensure_worker_scale(context, *, desired_workers, passthrough=None, worker_compose_file=None):
        calls["worker_scale"].append((context, desired_workers, passthrough, worker_compose_file))

    monkeypatch.setattr(app_module, "get_status_for_context", lambda ctx: down_status)
    monkeypatch.setattr(app_module, "get_status", lambda customer=None: up_response)
    monkeypatch.setattr(app_module, "resolve_vpn_profile", fake_resolve_vpn_profile)
    monkeypatch.setattr(app_module, "start_host_secure_client", fake_start_host_secure_client)
    ...
    assert calls["host_start"][0][1].profile_name == "e-Trikala VPN Profile Admins"
    assert calls["host_start"][0][2] == "yes"
    assert calls["worker_scale"] == [(context, 2, None, app_module.VPN_HOST_QUEUE_COMPOSE_FILE)]
