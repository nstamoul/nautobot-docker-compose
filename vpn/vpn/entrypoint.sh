...
CISCO_SECURE_CLIENT_AGENT="${CISCO_SECURE_CLIENT_AGENT:-/opt/cisco/secureclient/bin/vpnagentd}"
...
start_cisco_secure_client_agent() {
  if command -v dbus-daemon >/dev/null 2>&1 && ! pgrep -x dbus-daemon >/dev/null 2>&1; then
    log "Starting system DBus for Cisco Secure Client"
    mkdir -p /run/dbus
    dbus-daemon --system --fork || log "WARNING: system DBus failed to start"
  fi

  if command -v NetworkManager >/dev/null 2>&1 && ! pgrep -x NetworkManager >/dev/null 2>&1; then
    log "Starting NetworkManager for Cisco Secure Client"
    NetworkManager --no-daemon >/tmp/networkmanager.log 2>&1 &
    NETWORKMANAGER_PID=$!
    sleep 2
    if ! pgrep -x NetworkManager >/dev/null 2>&1 && ! kill -0 "${NETWORKMANAGER_PID}" >/dev/null 2>&1; then
      die "NetworkManager failed to start for Cisco Secure Client"
    fi
  fi

  if pgrep -x vpnagentd >/dev/null 2>&1; then
    log "Cisco Secure Client agent already running"
    return 0
  fi

  [[ -x "${CISCO_SECURE_CLIENT_AGENT}" ]] || die "Cisco Secure Client agent not executable at ${CISCO_SECURE_CLIENT_AGENT}"
  log "Starting Cisco Secure Client agent"
  "${CISCO_SECURE_CLIENT_AGENT}" &
  CISCO_AGENT_PID=$!
  sleep 2

  if ! pgrep -x vpnagentd >/dev/null 2>&1 && ! kill -0 "${CISCO_AGENT_PID}" >/dev/null 2>&1; then
    die "Cisco Secure Client agent failed to start"
  fi
}

ensure_cisco_secure_client_profile() {
  [[ -n "${CISCO_PROFILE_NAME:-}" ]] || return 0
  [[ -n "${FORTI_HOST:-}" ]] || return 0

  local profile_dir="/opt/cisco/secureclient/vpn/profile"
  local profile_file="${profile_dir}/shms-${CUSTOMER:-customer}.xml"
  mkdir -p "${profile_dir}"

  python3 - "${profile_file}" "${CISCO_PROFILE_NAME}" "${FORTI_HOST}" "${CISCO_USERGROUP:-}" <<'PY'
import sys
import xml.etree.ElementTree as ET

path, host_name, host_address, user_group = sys.argv[1:5]
root = ET.Element("AnyConnectProfile", {
    "xmlns": "http://schemas.xmlsoap.org/encoding/",
})
server_list = ET.SubElement(root, "ServerList")
host_entry = ET.SubElement(server_list, "HostEntry")
ET.SubElement(host_entry, "HostName").text = host_name
ET.SubElement(host_entry, "HostAddress").text = host_address
if user_group:
    ET.SubElement(host_entry, "UserGroup").text = user_group
tree = ET.ElementTree(root)
ET.indent(tree, space="  ")
tree.write(path, encoding="utf-8", xml_declaration=True)
PY
  chmod 0644 "${profile_file}"
  log "Ensured Cisco Secure Client profile '${CISCO_PROFILE_NAME}' at ${profile_file}"
}

run_cisco_secure_client() {
  [[ -n "${FORTI_USERNAME}" ]] || die "VPN username is required"
  [[ -n "${FORTI_PASSWORD}" ]] || die "VPN password is required"
  [[ -n "${CISCO_PROFILE_NAME:-${FORTI_HOST:-}}" ]] || die "Cisco Secure Client profile name or host is required"
  [[ -x "${CISCO_SECURE_CLIENT_BIN}" ]] || die "Cisco Secure Client CLI not executable at ${CISCO_SECURE_CLIENT_BIN}. Bundle the official client into the VPN image."

  local effective_banner_response="${CISCO_BANNER_RESPONSE:-${OPENCONNECT_BANNER_RESPONSE:-${VAULT_OPENCONNECT_BANNER_RESPONSE:-}}}"
  local target_profile="${CISCO_PROFILE_NAME:-${FORTI_HOST}}"

  setup_nat
  ensure_cisco_secure_client_profile
  start_cisco_secure_client_agent

  log "running: Cisco Secure Client profile '${target_profile}'"
  [[ -n "${CISCO_USERGROUP}" ]] && log "DEBUG: Cisco user group metadata present: '${CISCO_USERGROUP}'"
  print_runtime_command "${CISCO_SECURE_CLIENT_BIN}" -s connect "${target_profile}"

  local stdin_payload="${FORTI_USERNAME}"$'\n'"${FORTI_PASSWORD}"
  local stdin_debug="username, password(${#FORTI_PASSWORD} chars)"
  if [[ -n "${effective_banner_response}" ]]; then
    stdin_payload+=$'\n'"${effective_banner_response}"
    stdin_debug+=", banner_response"
  fi
  log "DEBUG: Stdin payload: ${stdin_debug}"

  local output
  if ! output=$(printf '%s\n' "${stdin_payload}" | "${CISCO_SECURE_CLIENT_BIN}" -s connect "${target_profile}" 2>&1); then
    printf '%s\n' "${output}" >&2
  ...
  log "Cisco Secure Client tunnel established, monitoring..."
  while true; do
    sleep 30
    if ! "${CISCO_SECURE_CLIENT_BIN}" state 2>/dev/null | grep -Eiq 'state:[[:space:]]*Connected|Connected'; then
      die "Cisco Secure Client tunnel down"
    fi
  done
}
  ...
  cisco-anyconnect)
    case "${VPN_CLIENT}" in
      openconnect)
        ...
        ;;
      cisco-secure-client)
        log "Starting Cisco AnyConnect with Cisco Secure Client"
        run_cisco_secure_client
        ;;
      *)
