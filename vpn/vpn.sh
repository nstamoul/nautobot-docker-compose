...
case "$cmd" in
    start)
        customer="${1:-${CUSTOMER:-}}"
        [[ -z "$customer" ]] && die "Usage: $0 start <customer> [--otp CODE] [--authgroup NAME] [--profile-name NAME] [--usergroup GROUP] [--banner-response TEXT] [--extra-args 'ARGS']"
        shift || true
...
Commands:
  start <customer> [--otp CODE] [--authgroup NAME] [--profile-name NAME] [--usergroup GROUP] [--banner-response TEXT] [--extra-args 'ARGS']
  start-worker
  stop
  stop-worker
  status
